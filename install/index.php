  ...
  $fp = fopen(dirname(__FILE__).'/seacms.sql','r');
	while(!feof($fp))
	{
		 ...
		 if(m_ereg(";$",$line))
		 {
			   ...
			   if($mysqlVersion < 4.1)
			   {
			   		$rs = mysqll_query($query,$conn);
			   }
			   else
			   {
			   		if(m_eregi('CREATE',$query))
			   		{
			   			$rs = mysqli_query($conn,m_eregi_replace('TYPE=MyISAM',$sql4tmp,$query));
			   		}
			   		else
			   		{
	...
	while(!feof($fp))
	{
		 ...
		 if(m_ereg(";$",$line))
		 {
			   $query .= $line;
			   $query = str_replace('sea_',$dbprefix,$query);
			   if($mysqlVersion < 4.1) $rs = mysqli_query($conn,$query);
	...
	$flinkquery = "INSERT INTO `{$dbprefix}flink` (`id`, `sortrank`, `url`, `webname`, `msg`, `email`, `logo`, `dtime`, `ischeck`) VALUES (NULL, '0', 'http://www.seacms.la', '海洋cms', '', '', '', '1432312055', '1');";
